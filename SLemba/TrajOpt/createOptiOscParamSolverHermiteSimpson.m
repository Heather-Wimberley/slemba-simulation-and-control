function out = createOptiOscParamSolverHermiteSimpson(N,N0,N1,N2,d)
...
[initCPG,initUPos] = seedOscillator(init.cpg0,N_hs,(tst_val+tsw_guess)/N_hs,osc_param);
...
initQ0 = seedCreationInput(init.q0,N0_hs,init.T0/N0_hs,init.u0);
initQ1 = seedCreationInput(init.q1,N1_hs,init.T1/N1_hs,init.u1);
initQ2 = seedCreationInput(init.q2,N2_hs,init.T2/N2_hs,init.u2);
...
tst = opti.variable(N_hs); opti.set_initial(tst,repmat(tst_val,N_hs,1)); opti.subject_to(repmat(0.02,N_hs,1)<=tst<=0.2*ones(N_hs,1));
tsw = opti.variable(N_hs); opti.set_initial(tsw,repmat(tsw_guess,N_hs,1)); opti.subject_to(repmat(1e-3,N_hs,1)<=tsw<=ones(N_hs,1));
kx = opti.variable(N_hs); opti.set_initial(kx,repmat(kx_val,N_hs,1)); opti.subject_to(repmat(0,N_hs,1)<=kx<=0.5*ones(N_hs,1));
% osc_param = [tst;tsw;kx];
% tsw = repmat(tsw_guess,N,1);
%tst = tst_val; kx = kx_val;% tsw = tsw_guess; 

opti.subject_to(tst<tsw);
...
q0 = opti.variable(10,N0_hs); opti.set_initial(q0,[init.q0,initQ0]);
opti.subject_to(repmat(LB.q,1,N0_hs)<=q0<=repmat(UB.q,1,N0_hs));
% dq0 = MX.zeros(10,N0_hs);
dq0 = opti.variable(10,N0_hs);
...
q1 = opti.variable(10,N1_hs); opti.set_initial(q1,[init.q1,initQ1]);
opti.subject_to(repmat(LB.q,1,N1_hs)<=q1<=repmat(UB.q,1,N1_hs));
% dq1 = MX.zeros(10,N1_hs); 
dq1 = opti.variable(10,N1_hs);
...
q2 = opti.variable(10,N2_hs); opti.set_initial(q2,[init.q2,initQ2]);
opti.subject_to(repmat(LB.q,1,N2_hs)<=q2<=repmat(UB.q,1,N2_hs));
% dq2 = MX.zeros(10,N2_hs); %
dq2 = opti.variable(10,N2_hs);
...
cpg0 = opti.variable(2,N0_hs); dcpg0 = opti.variable(2,N0_hs); opti.set_initial(cpg0,[init.cpg0,initCPG(:,1:N0_hs-1)]);
opti.subject_to(-1.3*ones(2,N0_hs)<=cpg0<=1.3*ones(2,N0_hs));
% cpg1 = opti.variable(2,N1); dcpg1 = opti.variable(2,N1); opti.set_initial(cpg1,[init.cpg1,initCPG1]);
cpg1 = opti.variable(2,N1_hs); dcpg1 = opti.variable(2,N1_hs); opti.set_initial(cpg1,initCPG(:,N0_hs:N0_hs+N1_hs-1));
opti.subject_to(-1.3*ones(2,N1_hs)<=cpg1<=1.3*ones(2,N1_hs));
% cpg2 = opti.variable(2,N2); dcpg2 = opti.variable(2,N2); opti.set_initial(cpg2,[init.cpg2,initCPG2]);
cpg2 = opti.variable(2,N2_hs); dcpg2 = opti.variable(2,N2_hs); opti.set_initial(cpg2,initCPG(:,N0_hs+N1_hs:end));
opti.subject_to(-1.3*ones(2,N2_hs)<=cpg2<=1.3*ones(2,N2_hs));

% intial state
opti.subject_to(q0(1,1) == 0); % x == 0
opti.subject_to(0.4 <= q0(2,1) <= 0.7); % y == 0.5
% opti.subject_to(q0(3,1) == th_calc(pi/2,0)); % th == 0.61
...
opti.subject_to(-0.2 <= cpg0(1,1) <= 0.2);
opti.subject_to(cpg0(2,1) >= 0.9);

% inputs for whole traj
u = opti.variable(3,N_hs); opti.subject_to(repmat(LB.u,1,N_hs)<=u<=repmat(UB.u,1,N_hs));
u_slack = opti.variable(3,N_hs); opti.subject_to(repmat(LB.u,1,N_hs)<=u_slack<=repmat(UB.u,1,N_hs)); % slack variable for output of mapping

% state of cpg (pos = 1, neg = 0)
u_x = opti.variable(N_hs); opti.subject_to(zeros(N_hs,1)<=u_x<=ones(N_hs,1));
opti.set_initial(u_x,[0,initUPos(1,:)]);
u_y = opti.variable(N_hs); opti.subject_to(zeros(N_hs,1)<=u_y<=ones(N_hs,1));
...
lambda_grf = opti.variable(3,N_hs); % grfx_pos, grfx_neg, grfy
lambda_fend = opti.variable(2,N_hs); % fendr, fende
...
for k = 1:N0_hs
    ...
    % dq0(:,k) = dq_ode_in_comp(q0(:,k),u(:,k),lambda);
...
for k1 = 1:N1_hs
    ...
    % dq1(:,k1) = dq_ode_in_comp(q1(:,k1),u(:,k),lambda);
...
for k2 = 1:N2_hs
    ...
    % dq2(:,k2) = dq_ode_in_comp(q2(:,k2),u(:,k),lambda);
...
for k = (1:N0-1)*d - 1 
    k_n = (k + 1)/d;
    opti.subject_to(q0(:,k+d) == q0(:,k) + (1/6)*h0(k_n)*(dq0(:,k) + 4*dq0(:,k+d/2) + dq0(:,k+d)));
    opti.subject_to(q0(:,k+d/2) == (1/2)*(q0(:,k) + q0(:,k+d)) + (h0(k_n)/8)*(dq0(:,k) - dq0(:,k+d)));

    opti.subject_to(cpg0(:,k+d) == cpg0(:,k) + (1/6)*h0(k_n)*(dcpg0(:,k) + 4*dcpg0(:,k+d/2) + dcpg0(:,k+d)));
    opti.subject_to(cpg0(:,k+d/2) == (1/2)*(cpg0(:,k) + cpg0(:,k+d)) + (h0(k_n)/8)*(dcpg0(:,k) - dcpg0(:,k+d)));
end

% between state node conditions
opti.subject_to(q1(:,1) == q0(:,end-1) + (1/6)*h0(end)*(dq0(:,end-1) + 4*dq0(:,end) + dq1(:,1)));
opti.subject_to(q0(:,end) == (1/2)*(q0(:,end-1) + q1(:,1)) + (h0(end)/8)*(dq0(:,end-1) - dq1(:,1)));

opti.subject_to(cpg1(:,1) == cpg0(:,end-1) + (1/6)*h0(end)*(dcpg0(:,end-1) + 4*dcpg0(:,end) + dcpg1(:,1)));
opti.subject_to(cpg0(:,end) == (1/2)*(cpg0(:,end-1) + cpg1(:,1)) + (h0(end)/8)*(dcpg0(:,end-1) - dcpg1(:,1)));

for k = (1:N1-1)*d - 1 
    k_n = (k + 1)/d;
    opti.subject_to(q1(:,k+d) == q1(:,k) + (1/6)*h1(k_n)*(dq1(:,k) + 4*dq1(:,k+d/2) + dq1(:,k+d)));
    opti.subject_to(q1(:,k+d/2) == (1/2)*(q1(:,k) + q1(:,k+d)) + (h1(k_n)/8)*(dq1(:,k) - dq1(:,k+d)));

    opti.subject_to(cpg1(:,k+d) == cpg1(:,k) + (1/6)*h1(k_n)*(dcpg1(:,k) + 4*dcpg1(:,k+d/2) + dcpg1(:,k+d)));
    opti.subject_to(cpg1(:,k+d/2) == (1/2)*(cpg1(:,k) + cpg1(:,k+d)) + (h1(k_n)/8)*(dcpg1(:,k) - dcpg1(:,k+d)));
end

% between state node conditions
opti.subject_to(q2(:,1) == q1(:,end-1) + (1/6)*h1(end)*(dq1(:,end-1) + 4*dq1(:,end) + dq2(:,1)));
opti.subject_to(q1(:,end) == (1/2)*(q1(:,end-1) + q2(:,1)) + (h1(end)/8)*(dq1(:,end-1) - dq2(:,1)));

opti.subject_to(cpg2(:,1) == cpg1(:,end-1) + (1/6)*h1(end)*(dcpg1(:,end-1) + 4*dcpg1(:,end) + dcpg2(:,1)));
opti.subject_to(cpg1(:,end) == (1/2)*(cpg1(:,end-1) + cpg2(:,1)) + (h1(end)/8)*(dcpg1(:,end-1) - dcpg2(:,1)));

for k = (1:N2-1)*d - 1 
    k_n = (k + 1)/d;
    opti.subject_to(q2(:,k+d) == q2(:,k) + (1/6)*h2(k_n)*(dq2(:,k) + 4*dq2(:,k+d/2) + dq2(:,k+d)));
    opti.subject_to(q2(:,k+d/2) == (1/2)*(q2(:,k) + q2(:,k+d)) + (h2(k_n)/8)*(dq2(:,k) - dq2(:,k+d)));

    opti.subject_to(cpg2(:,k+d) == cpg2(:,k) + (1/6)*h2(k_n)*(dcpg2(:,k) + 4*dcpg2(:,k+d/2) + dcpg2(:,k+d)));
    opti.subject_to(cpg2(:,k+d/2) == (1/2)*(cpg2(:,k) + cpg2(:,k+d)) + (h2(k_n)/8)*(dcpg2(:,k) - dcpg2(:,k+d)));
end

%% Contact Order & transition conditions
for k = 1:N0_hs
...
for k = N0_hs+1:N0_hs + N1_hs
    opti.subject_to(lambda_grf(3,k)>0);
end

% piston does not reach end stop
opti.subject_to(q1(4,end)<UB.q(4));
opti.subject_to(q2(4,1)<UB.q(4));

for k = N0_hs + N1_hs+1:N_hs
    opti.subject_to(lambda_grf(3,k)<=0);
end
...
P_xpos = opti.variable(N_hs); opti.subject_to(P_xpos >= zeros(N_hs,1));
P_xneg = opti.variable(N_hs); opti.subject_to(P_xneg >= zeros(N_hs,1));
P_ypos = opti.variable(N_hs); opti.subject_to(P_ypos >= zeros(N_hs,1));
P_yneg = opti.variable(N_hs); opti.subject_to(P_yneg >= zeros(N_hs,1));

x_pos = opti.variable(N_hs); opti.subject_to(x_pos >= zeros(N_hs,1));
x_neg = opti.variable(N_hs); opti.subject_to(x_neg >= zeros(N_hs,1));
y_pos = opti.variable(N_hs); opti.subject_to(y_pos >= zeros(N_hs,1));
y_neg = opti.variable(N_hs); opti.subject_to(y_neg >= zeros(N_hs,1));

b_posux = opti.variable(N_hs); opti.subject_to(b_posux >= zeros(N_hs,1));
b_negux = opti.variable(N_hs); opti.subject_to(b_negux >= zeros(N_hs,1));

b_posuy = opti.variable(N_hs); opti.subject_to(b_posuy >= zeros(N_hs,1));
b_neguy = opti.variable(N_hs); opti.subject_to(b_neguy >= zeros(N_hs,1));
...
%% Complimentarity
P_fende = opti.variable(N_hs); P_fendr = opti.variable(N_hs);
P_grfx_pos = opti.variable(N_hs); P_grfx_neg = opti.variable(N_hs); P_grfy = opti.variable(N_hs); P_fric = opti.variable(N_hs);
opti.subject_to(P_fendr>=zeros(N_hs,1));
opti.subject_to(P_fende>=zeros(N_hs,1));
opti.subject_to(P_grfx_pos>=zeros(N_hs,1));
opti.subject_to(P_grfx_neg>=zeros(N_hs,1));
opti.subject_to(P_grfy>=zeros(N_hs,1));
opti.subject_to(P_fric>=zeros(N_hs,1));

% complemntarity slack variables
a_pistonr = opti.variable(1,N_hs); opti.subject_to(a_pistonr>=zeros(1,N_hs));
b_pistonr = opti.variable(1,N_hs); opti.subject_to(b_pistonr>=zeros(1,N_hs));
a_pistone = opti.variable(1,N_hs); opti.subject_to(a_pistone>=zeros(1,N_hs));
b_pistone = opti.variable(1,N_hs); opti.subject_to(b_pistone>=zeros(1,N_hs));

a_foot = opti.variable(1,N_hs); opti.subject_to(a_foot>=zeros(1,N_hs));
b_foot = opti.variable(1,N_hs); opti.subject_to(b_foot>=zeros(1,N_hs));
a_friction = opti.variable(1,N_hs); opti.subject_to(a_friction>=zeros(1,N_hs));
b_friction = opti.variable(1,N_hs); opti.subject_to(b_friction>=zeros(1,N_hs));
a_slip_p = opti.variable(1,N_hs); opti.subject_to(a_slip_p>=zeros(1,N_hs));
b_slip_p = opti.variable(1,N_hs); opti.subject_to(b_slip_p>=zeros(1,N_hs));
a_slip_n = opti.variable(1,N_hs); opti.subject_to(a_slip_n>=zeros(1,N_hs));
b_slip_n = opti.variable(1,N_hs); opti.subject_to(b_slip_n>=zeros(1,N_hs));

% Posa Method
gamma = opti.variable(1,N_hs); opti.subject_to(gamma>=zeros(1,N_hs));
...
% J_u = J_u + sum(u_n(1,:)/(m_avg*9.81).^2);
J_u = J_u + sum(u_n(1,:)/(40).^2);
% J_u = J_u + 1e3*sum((u_n(2,:).*h').^2);
% J_u = J_u + sum((1e1*kx).^2);
J_u = J_u + 1e1*sum((tst/0.1).^2);
